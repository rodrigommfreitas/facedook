...
export const Post = ({ post }: Props) => {
  ...
  return (
    <>
      {isDeleting && (
        <Delete postId={post._id} toggleDeleting={toggleDeleting} />
      )}
      <div className='w-full bg-white pt-3 pb-1 sm:rounded-lg shadow-sm shadow-gray-300'>
        <div className='flex px-4 justify-between items-center'>
          ...
          {currentUser?._id === user._id && (
            <button
              ...
              className='h-fit w-fit text-gray-dark rounded-full p-2 hover:bg-gray-light hover:text-red-500 active:bg-gray-300 active:scale-95 transition-all '
            >
              <TrashIcon height='0.875em' width='0.875em' />
            </button>
          )}
        </div>
        ...
        {post.img && (
          <img
            className='border-t border-b border-gray-light w-full h-fit max-h-[700px]'
            src={PF + post.img}
            alt=''
          />
        )}
